...
interface IChatParticipantDisambiguation {
	category: string;
	description: string;
	examples: string[];
}

interface IChatParticipantCommand {
	name: string;
	description?: string;
	when?: string;
	sampleRequest?: string;
	isSticky?: boolean;
	disambiguation?: IChatParticipantDisambiguation[];
}

interface IChatParticipantContribution {
	id: string;
	name: string;
	fullName?: string;
	description?: string;
	...
	when?: string;
	disambiguation?: IChatParticipantDisambiguation[];
	commands?: IChatParticipantCommand[];
	isDefault?: boolean;
	isAgent?: boolean;
	locations?: string[];
}

const chatParticipantExtensionPoint = extensionsRegistry.ExtensionsRegistry.registerExtensionPoint<IChatParticipantContribution[]>({
	...
	jsonSchema: {
		...
		items: {
			...
			properties: {
				...
				isDefault: {
					description: localize('chatParticipantIsDefault', "Whether this participant should be the default one."),
					type: 'boolean'
				},
				isAgent: {
				...
				locations: {
					description: localize('chatParticipantLocations', "The locations where this participant can be used."),
					type: 'array',
					items: {
						type: 'string'
					}
		...
		chatParticipantExtensionPoint.setHandler((extensions, delta) => {
			for (const extension of delta.added) {
				for (const providerDescriptor of extension.value) {
					...
					const participantsDisambiguation: IChatParticipantDisambiguation[] = [];
