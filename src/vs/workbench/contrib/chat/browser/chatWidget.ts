...
interface IChatWidgetLocationOptions {
	autoScroll?: (mode: ChatMode) => boolean;
	renderFollowups?: boolean;
	supportsFileReferences?: boolean;
	supportsAdditionalParticipants?: boolean;
	rendererOptions?: {
		renderTextEditsAsSummary?: (uri: URI) => boolean;
		referencesExpandedWhenEmptyResponse?: boolean;
		progressMessageAtBottomOfResponse?: (mode: ChatMode) => boolean;
	};
...
interface IChatWidgetViewContext {
	viewId: string;
}
...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	private readonly _onDidChangeAgent = this._register(new Emitter<{ agent: IChatAgentData; slashCommand?: IChatAgentCommand }>());
	readonly onDidChangeAgent = this._onDidChangeAgent.event;

	private readonly _onDidFocus = this._register(new Emitter<void>());
	readonly onDidFocus = this._onDidFocus.event;

	private readonly _onDidChangeViewModel = this._register(new Emitter<void>());
	...
	private readonly _onDidChangeParsedInput = this._register(new Emitter<void>());
	...
	constructor(
		...
		this._location = typeof location === 'string' ? { location } : location;
		this.viewContext = _viewContext ?? { viewId: viewOptions.viewId };
		...
		this.agentInInput = ChatContextKeys.agentInInput.bindTo(contextKeyService);

		this._codeBlockModelCollection = this._register(new CodeBlockModelCollection());
