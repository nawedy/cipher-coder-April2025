...
export class ChatService extends Disposable implements IChatService {
	...
	constructor(
		...
		this._chatServiceTelemetry = this.instantiationService.createInstance(new SyncDescriptor(ChatServiceTelemetry, [], true));
		...
		this._chatSessionStore = this._register(this.instantiationService.createInstance(new SyncDescriptor(ChatSessionStore, [], true)));
	...
	private _startSession(someSessionHistory: IExportableChatData | ISerializableChatData | undefined, location: ChatAgentLocation, isGlobalEditingSession: boolean, token: CancellationToken): ChatModel {
		const model = this.instantiationService.createInstance(new SyncDescriptor(ChatModel, [someSessionHistory, location], true));
	...
	private parseChatRequest(sessionId: string, request: string, location: ChatAgentLocation, options: IChatSendRequestOptions | undefined): IParsedChatRequest {
		...
		const parsedRequest = this.instantiationService.createInstance(new SyncDescriptor(ChatRequestParser, [], true)).parseChatRequest(sessionId, request, location, parserContext);
	...
	private _sendRequestAsync(model: ChatModel, sessionId: string, parsedRequest: IParsedChatRequest, attempt: number, enableCommandDetection: boolean, defaultAgent: IChatAgent, location: ChatAgentLocation, options?: IChatSendRequestOptions): IChatSendRequestResponseState {
		...
		this._pendingRequests.set(model.sessionId, this.instantiationService.createInstance(new SyncDescriptor(CancellableRequest, [source, undefined], true)));
	...
	async addCompleteRequest(sessionId: string, message: IParsedChatRequest | string, variableData: IChatRequestVariableData | undefined, attempt: number | undefined, response: IChatCompleteResponse): Promise<void> {
		...
		const parsedRequest = typeof message === 'string' ?
			this.instantiationService.createInstance(new SyncDescriptor(ChatRequestParser, [], true)).parseChatRequest(sessionId, message) :
			message;
