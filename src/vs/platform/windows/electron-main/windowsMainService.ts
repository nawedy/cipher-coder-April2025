	...
	private async doResolveFilePath(path: string, options: IPathResolveOptions, skipHandleUNCError?: boolean): Promise<IPathToOpen<ITextEditorOptions> | undefined> {
		...
		try {
			...
			if (error instanceof Error) {
				...
				this.logService.error(`Invalid path provided: ${path}, Unknown error`);
			}
		}
	...
	private async openInBrowserWindow(options: IOpenBrowserWindowOptions): Promise<ICodeWindow> {
		...
		if (!window) {
			...
			const createdWindow = window = this.instantiationService.createInstance(
				new SyncDescriptor(CodeWindow, [{
					state,
					workspace: configuration.workspace,
					remoteAuthority: configuration.remoteAuthority,
					isExtensionDevelopmentHost: !!configuration.extensionDevelopmentPath,
					isExtensionTestHost: !!configuration.extensionTestsPath,
					filesToOpenOrCreate: configuration.filesToOpenOrCreate,
					filesToDiff: configuration.filesToDiff,
					filesToMerge: configuration.filesToMerge,
					filesToWait: configuration.filesToWait,
					logLevel: configuration.logLevel,
					loggers: configuration.loggers,
					fullscreen: configuration.fullscreen,
					maximized: configuration.maximized,
					accessibilitySupport: configuration.accessibilitySupport,
					colorScheme: configuration.colorScheme,
					autoDetectHighContrast: configuration.autoDetectHighContrast,
					autoDetectColorScheme: configuration.autoDetectColorScheme,
					isCustomZoomLevel: configuration.isCustomZoomLevel,
					perfMarks: configuration.perfMarks,
					os: configuration.os,
					policiesData: configuration.policiesData
				}], true)
			);
