...
export class MenubarMainService extends Disposable implements IMenubarMainService {
	...
	private async installMenuBarAfterWindowOpen(): Promise<Menubar> {
		...
		const descriptor = new SyncDescriptor(Menubar, [], true);
		const menubar = this.instantiationService.createInstance(descriptor);
		this._register(menubar);
		return menubar as Menubar;
	}
